...
public class HealingZone : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        if (IsClient)
        {
            HealPower.OnValueChanged += HandleHealPoweChanged;
            HandleHealPoweChanged(0, HealPower.Value);
        }
        if (IsServer)
        {
    ...
    private void HandleHealPoweChanged(int oldHealPower, int newHealPower)
    {
